...
const Charts = (() => {
  ...
  function area(container, data, opts = {}) {
    ...
    for (let g = 0; g <= 3; g++) {
      const y = padT + (ih / 3) * g;
      $('line', { x1: padL, y1: y, x2: W - padR, y2: y, stroke: 'rgba(255,255,255,.07)', 'stroke-width': 1 }, svg);
      const val = Math.round(max * (1 - g / 3));
      const t = $('text', { x: padL - 8, y: y + 4, 'text-anchor': 'end', 'font-size': 10, fill: '#5b6478' }, svg);
    ...
    // dots
    data.forEach((v, i) => {
      const dot = $('circle', { cx: px(i), cy: py(v), r: 3, fill: opts.color || '#10b981', stroke: '#0b1120', 'stroke-width': 2 }, svg);
      dot.style.opacity = 0;
    ...
    if (opts.labels) {
      ...
      opts.labels.forEach((lb, i) => {
        if (i % step !== 0) return;
        const t = $('text', { x: px(i), y: H - 8, 'text-anchor': 'middle', 'font-size': 9.5, fill: '#5b6478' }, svg);
        t.textContent = lb;
      });
    }
  }
  ...
  function donut(container, items) {
    ...
    const t1 = $('text', { x: cx, y: cy - 2, 'text-anchor': 'middle', 'font-size': 26, 'font-weight': 800, fill: '#eef2ff', 'font-family': 'Sora' }, svg);
    t1.textContent = total + '%';
    const t2 = $('text', { x: cx, y: cy + 18, 'text-anchor': 'middle', 'font-size': 10, fill: '#8b93a7' }, svg);
  ...
  function bars(container, labels, series, opts = {}) {
    ...
    for (let g = 0; g <= 3; g++) {
      const y = padT + (ih / 3) * g;
      $('line', { x1: padL, y1: y, x2: W - padR, y2: y, stroke: 'rgba(255,255,255,.07)' }, svg);
    }
    ...
    labels.forEach((lb, i) => {
      ...
      const t = $('text', { x: gx + groupW / 2, y: H - 10, 'text-anchor': 'middle', 'font-size': 9.5, fill: '#5b6478' }, svg);
      t.textContent = lb;
    });
  }
